...
export async function processStripeWebhook(
  ...
  // Biến lưu job ID để xử lý sau transaction
  let kitchenJobId: bigint | null = null;
  ...
  try {
    const result = await db.$transaction(
      async (tx) => {
        ...
        if (existing) {
          console.log(
            '[Stripe Webhook] Existing event found',
            {
              ...
              paymentId: existing.payment_id?.toString() ?? null,
            }
          );
          ...
          if (existing.processed === 1) {
            ...
            return {
              duplicate: true,
              processed: true,
              paymentId: existing.payment_id?.toString(),
            };
          }
        ...
        if (!webhook) {
          webhook = await webhookRepository.createWebhook(tx, {
            eventId: event.id,
            eventType: event.type,
            payload: rawPayload,
          });
        ...
        const metadataOrderId = intent.metadata?.order_id;
        ...
        let payment = await tx.tbl_order_payments.findFirst({
          where: {
        ...
        if (!payment && metadataPaymentId) {
          ...
          try {
            metadataPaymentIdBigInt = BigInt(metadataPaymentId);
          } catch {
            throw new Error(
          ...
          payment = await tx.tbl_order_payments.findUnique({
            where: {
        ...
        const stripeAmount = Number(intent.amount ?? 0);
        const paymentAmount = Number(
          payment.amount?.toString() ?? 0
        );

        const stripeCurrency = (intent.currency || '').toUpperCase();
        const paymentCurrency = (payment.currency || 'JPY').toUpperCase();

        const matchesOrderId =
          !metadataOrderId ||
          metadataOrderId === payment.order_id.toString();

        const matchesPaymentId =
          !metadataPaymentId ||
          metadataPaymentId === payment.id.toString();

        const amountMatches = stripeAmount === paymentAmount;
        const currencyMatches = stripeCurrency === paymentCurrency;
        ...
        console.log(
          '[Stripe Webhook] Payment validation',
          {
            ...
            metadata: {
              ...
              paymentId: metadataPaymentId ?? null,
              orderMatches: matchesOrderId,
              paymentMatches: matchesPaymentId,
            },
          }
        );
        ...
        if (
          ...
          console.error(
            '[Stripe Webhook] PAYMENT VALIDATION FAILED',
            {
              ...
              actualCurrency: stripeCurrency,
              expectedOrderId: payment.order_id.toString(),
              metadataOrderId,
              expectedPaymentId: payment.id.toString(),
              metadataPaymentId,
            }
          );
        ...
        if (event.type === 'payment_intent.succeeded') {
          ...
          const updatedPayment = await tx.tbl_order_payments.findUnique({
            where: { id: payment.id },
          });
          ...
          console.log(
            '[Stripe Webhook] STEP 1 RESULT: Payment after update',
            {
              ...
              transactionId: updatedPayment?.transaction_id ?? null,
            }
          );
          ...
          const order = await tx.tbl_customer_orders.findUnique({
          ...
          const updatedOrder = await tx.tbl_customer_orders.findUnique({
            where: { id: order.id },
          });
          ...
          console.log(
            ...
            {
              orderId: updatedOrder?.id.toString() ?? null,
              orderToken: updatedOrder?.order_token ?? null,
              status: updatedOrder?.status ?? null,
              paidAt: updatedOrder?.paid_at ?? null,
              updatedAt: updatedOrder?.updated_at ?? null,
            }
          );
          ...
          /*
           * ==========================================================
           * 9.5 CREATE KITCHEN PRINT JOB RECORD (TRONG TRANSACTION)
           * ==========================================================
           * Chỉ tạo record, không gọi máy in.
           */
          console.log(
            '[Stripe Webhook] STEP 3.5: Creating KITCHEN print job record (pending)',
            {
              orderId: order.id.toString(),
          ...
          console.log(
            '[Stripe Webhook] STEP 3.5 RESULT: KITCHEN print job record created',
            {
              orderId: order.id.toString(),
              created: printJobResult.created,
              printJobId: printJobResult.job.id.toString(),
              status: printJobResult.job.status,
            }
          );

          // Lưu job ID để xử lý sau transaction
          kitchenJobId = printJobResult.job.id;
        ...
        if (event.type === 'payment_intent.payment_failed') {
          ...
          const failure = getStripePaymentFailure(intent);
          ...
          console.log(
            '[Stripe Webhook] Failed payment result',
            {
              paymentId: payment.id.toString(),
              status: updatedPayment?.status ?? null,
              failureCode: updatedPayment?.failure_code ?? null,
            }
          );
          ...
          if (!updatedOrder || updatedOrder.status !== 'PAYMENT_FAILED') {
            throw new Error(
        ...
        if (event.type === 'payment_intent.canceled') {
          ...
          if (!updatedPayment || updatedPayment.status !== 'CANCELLED') {
            throw new Error(
        ...
        throw new Error(`Unhandled PaymentIntent event: ${event.type}`);
      },
      {
        maxWait: 10000,
        timeout: 20000,
    ...
    // ============================================================
    // SAU KHI TRANSACTION COMMIT: KHÔNG GỌI IN Ở ĐÂY
    // ============================================================
    if (kitchenJobId) {
      console.log(
        `[Webhook] Print job ${String(kitchenJobId)} created (status: WAITING). Worker will handle it.`
      );
      // Worker (Raspberry Pi) sẽ poll và in
    }
    ...
    if (
      ...
      if (targetString.includes('event_id')) {
        console.warn(
